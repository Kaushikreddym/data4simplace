...
#: The engine wraps its day-of-year at 365 (``torchcrop_run.run_batch``), and
#: CyBench's fractional ``sos``/``eos`` follow the same convention. Using 365.25
#: here would put a slow drift into every circular difference.
DAYS_IN_YEAR: Final[float] = 365.0

#: TorchCrop's sowing day-of-year — a **constant input**, not a prediction. The
#: SIMPLACE export carries no sowing calendar, so `torchcrop_run.py` sows every
#: cell from Crete to Lapland on the same day. Keep this in sync with
#: ``TC_SOWING_DOY`` if the run is repeated with a different latch.

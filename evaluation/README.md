...
Evaluates the 10 km [TorchCrop](../submit/TORCHCROP.md) winter wheat run over
...
Every input path in [`utils/config.py`](utils/config.py) can be overridden by an
...
* **Flowering is absent from both sides.** `torchcrop_run.run_batch` discards
...
interpolated onto the 10 km grid.** `utils.grid.bin_cells` averages whichever
10 km cropland cells fall inside each 0.5° cell (`utils.grid.snap_to_grid`),
...
interannual term (`utils.metrics.to_anomalies`, each cell's deviation from its
...
`utils.doy.doy_in_window` and `window_position` test whether the simulated date

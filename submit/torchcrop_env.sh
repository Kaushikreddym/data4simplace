...
# --- Project & environment ---------------------------------------------------
export TC_PROJECT_DIR="${TC_PROJECT_DIR:-/data01/FDS/muduchuru/codes/GITHUB/data4simplace}"
...
tc_activate() {
    ...
    [ "${had_u}" -eq 1 ] && set -u
    python -c "import torch, torchcrop" 2>/dev/null || {
        echo "ERROR: torch/torchcrop not importable in env '${TC_CONDA_ENV}'." >&2
        echo "       Install with: pip install -e /beegfs/muduchuru/pkgs_fnl/torchcrop" >&2
        exit 1
    }
}

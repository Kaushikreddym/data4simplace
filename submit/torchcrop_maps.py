...
Reads every ``torchcrop_shard_*.parquet`` written by `torchcrop_run`, checks
...
from torchcrop_run import (  # noqa: E402
    GRID_MAX_LAT, GRID_MIN_LON, GRID_RES, N_LAT, N_LON, id_to_rowcol,
)

logger = logging.getLogger("torchcrop_maps")
...
def to_grid(frame: pd.DataFrame) -> xr.Dataset:
    """Scatter the per-(cell, year) rows onto the ``(year, lat, lon)`` grid.

    Placement is arithmetic — ``SimplaceID`` encodes the row and column
    directly — so this is an index assignment, not a spatial join.
    """
    years = np.sort(frame["year"].unique())
    year_idx = pd.Series(np.arange(len(years)), index=years)
    row, col = id_to_rowcol(frame["SimplaceID"].to_numpy())
    yr = year_idx.reindex(frame["year"].to_numpy()).to_numpy()

    lats = GRID_MAX_LAT - GRID_RES / 2.0 - np.arange(N_LAT) * GRID_RES
    lons = GRID_MIN_LON + GRID_RES / 2.0 + np.arange(N_LON) * GRID_RES
